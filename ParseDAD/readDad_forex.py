...
def tick_to_time(tick, is_night):
    ...
    datetime_begin = datetime.datetime(2010, 1, 4, 9, 0)
    if is_night:
        datetime_begin = datetime.datetime(2010, 1, 4, 6, 0)
            ...
            for min_tick in day_ticks:
                ...
                if time_index >= len(day_mins):
                    ...
                    time_str = "%d/%02d/%02d-%02d:%02d" % (min_date_time.year, min_date_time.month,
                                                           min_date_time.day, min_date_time.hour,
                                                           min_date_time.minute)
                    tick_str = "%.4f\t%.4f\t%.4f\t%.4f\t%.4f" % (min_tick[2], min_tick[3],
