...
class exchange_config(object):
	...
	def is_in_exchange_time(this, contract, min):
		...
		for key in info.stage_time.keys():
			if key == 99999999:
				time_arr_list = info.stage_time[key]
				for time_arr in time_arr_list:
					if min > time_arr[0] and min <= time_arr[1]:
						return True

		return False

	def is_in_exchange_time(this, contract, date, min):
		exg_sub = this.contract_exchg_sub[contract]
		info = this.exchg_info[exg_sub]
		for key in info.stage_time.keys():
			if date < key:
	...
	def at_end_min(this, contract, date, min):
		exg_sub = this.contract_exchg_sub[contract]
		info = this.exchg_info[exg_sub]
		for key in info.stage_time.keys():
			if date < key:
...
def test():
	...
	print(config.get_contract_daymin("ru", 20100611))
