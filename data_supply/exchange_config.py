...
class exchange_config(object):
    ...
    def is_in_exchange_time(this, contract, min):
        exg_sub = this.contract_exchg_sub[contract]
        info = this.exchg_info[exg_sub]
        for key in info.stage_time.keys():
            if 20000000 + min / 10000 < key:
                time_arr_list = info.stage_time[key]
                for time_arr in time_arr_list:
                    if min % 10000 >= time_arr[0] and min % 10000 <= time_arr[1]:
                        return True
                break

        return False

    def get_contract_min_area(this, contract, date):
        set_time_area = []

        exg_sub = this.contract_exchg_sub[contract]
        info = this.exchg_info[exg_sub]
        keys = info.stage_time.keys()
        keys.sort()
        for end_date in keys:
            if date < end_date:
                time_arr_list = info.stage_time[end_date]
                for time_arr in time_arr_list:
                    set_time_area.append(time_arr)
                break

        return set_time_area
