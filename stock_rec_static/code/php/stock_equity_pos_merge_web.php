...
function backtest_trading($db, $trading_days, $datas, $output_param, $houseware, &$log){
            ...
            foreach ($date_datas as $data) {
                ...
                switch($data["type"]){
                    ...
                    case "allotment": // 配股
                        ...
                        $day_log[$date][] = sprintf("送股/配股：%s %d", $stock_name, $amount);
                        break;
...
function show_calc(){
    ...
    try{
        ...
        $data_result = backtest_trading($db_price, $trading_days, $new_datas, '*', $filename_utf8, $log_set);
        ...
        // 为合约表格输出
        $contract_table_res = get_contract_table_res($new_stock_names, $data_result);
        ...
        // 组织最终串
        $series_str .= sprintf("{name:'总资金',data:[%s]},", $day_profit_str);
        foreach($contract_strs as $C=>$str){
            if(array_key_exists($C, $new_stock_names)){
                $series_str .= sprintf("{name:'%s %s',data:[%s]},", $C, $new_stock_names[($C)], $str);
            }
            else {
