...
def main():
    cur_path = os.getcwd()
    #在根目录下寻找策略名和目标数据文件
    result_file_info = Base.find_test_result_file(cur_path, "margin")
    ...
    for contract in map_test_file:
        ...
        for item in test_result_files.items():
            datas_margin = Base.read_margin(item[1])
            Base.merge_datas(time_set, contract_all_datas, item[0], datas_margin)

        #生成目标网页
        print contract
        web_path = Base.get_web_path(cur_path, policy, contract, "margin")
        Base.create_result_web(web_path, contract, time_set, "利润", contract_all_datas)
